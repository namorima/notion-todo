...
export async function handler(event, context) {
  ...
  try {
    ...
    if (!response.ok) {
      return createResponse(response.status, { error: 'Failed to mark calendar event as done', details: data });
    }
    ...
    return createResponse(500, { error: 'Error updating calendar event' });
  }
}
